...
Six distros are supported: *Arch*, *CentOS* (9), *Fedora* (36 and the default 37), *Gentoo*, *openSUSE* (Tumbleweed), and *Ubuntu* (22.04 and the default 22.10).  This installer only implements features as supported in the distros themselves; i.e. it does not build newer package versions or take better tools from other distros to accomplish tasks.  As such, a different feature set is available depending on the distro choice.  The following describes the level of support for some of the major features across distros.
